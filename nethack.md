...
:    Отображать искры когда монстр или вы получает удар, к которому имеет
     сопротивление (по умолчанию включено).
...
:    Подавлять сообщения об изменениях в версиях ниже текущей
     (например «`suppress_alert:3.3.1`»).
...
:    Когда приостанавливается игра для отображения эффектов, например взрывов
     или движущихся объектов, не отображать дополнительные символы а переждать.
...
:    Отображать завершающее окно в NetHack, а не в стандартном
     потоке вывода (по умолчанию включено).

     Если эта установка включена, то вы сможете увидеть топ лист в окне NetHack,
	 когда он запущен без родительского окна, но не увидите после окончания
	 игры на терминале или эмуляторе.

`traps`

:    Устанавливает графические символы для отображения ловушек
     (по умолчанию «`^^^^^^^^^^^^^^^^^"^^^^`»).
...
:   Разрешить перемещения на дальние расстояния (по умолчанию включено).
    ...
    Выключение этой опции не даст вашему персонажу неумышленно переместиться,
	когда вы случайно кликните мышью по карте.
...
:   Выбор оконной системы для использования, например «`tty`» или «`X11`»
    (настройки по умолчанию зависят от версии).
...
Вы можете выяснить, поддерживается ли опция, которую используете, с
...
:   Позиция отображения сообщений (`top` — вверху, `bottom` — внизу,
    `left` — слева, `right` — справа).

`align_status`

:   Позиция отображения текущего состояния
    (`top` — вверху, `bottom` — внизу, `left` — слева, `right` — справа).
...
:   NetHack будет передавать восьмибитные символы (к примеру, указанные в
    настройках ловушек) напрямую на терминал (по умолчанию выключено).
...
:   Отображать визуальное отличие питомца от других животных.
    ...
    Эта опция работает по разному, в зависимости от того, какой
	тип оконного режима используется.

    При текстовом отображении, символ подсвечивается. В случае с
	тайлами, обычно отображается символ сердечка рядом с питомцами.
...
:   Использовать всплывающие окна или спрашивать в текстовом режиме,
    при вопросах к пользователю.
